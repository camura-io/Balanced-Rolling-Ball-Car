...
#define MODE56_TARGET_SPEED_CMS   25.0f   /* 正常循迹巡航目标速度：25 cm/s */
#define MODE56_START_SPEED_CMS     5.0f   /* 起步加速初速度：5 cm/s */
#define MODE56_MIN_SPEED_CMS       5.0f   /* 减速刹车下限速度：5 cm/s，降至此值直接全停 */

/* 
 * 斜坡加减速步长（基于 10 ms 控制周期）：
 * - 0.02f cm/s 对应加速度 a = 0.02 / 0.01s = 2 cm/s^2 (需 10.0s 从 5 达到 25cm/s)
 * - 0.04f cm/s 对应减速度 a = 0.04 / 0.01s = 4 cm/s^2 (需 5.0s 从 25cm/s 降至 5cm/s)
 */
#define MODE56_ACCEL_STEP_CMS     0.02f   /* 10 ms 周期的加速步长 */
#define MODE56_DECEL_STEP_CMS     0.02f   /* 10 ms 周期的减速步长 */


#define TRACK_SLOW_DISTANCE_CM    580.0f
/* === 模式 4 参数配置 === */
#define MODE4_TEST_SPEED_CMS      30.0f   /* 模式 4 巡航目标速度：30 cm/s */
#define MODE4_TEST_DISTANCE_CM    150.0f  /* 模式 4 计时器停止距离：150 cm */
#define MODE4_DECEL_DISTANCE_CM   200.0f  /* 模式 4 开始减速距离：200 cm */
#define MODE4_START_SPEED_CMS     10.0f   /* 模式 4 起步初速度（恢复最初设置的10cm/s） */
#define MODE4_ACCEL_STEP_CMS      0.04f   /* 模式 4 加速步长 */
#define MODE4_DECEL_STEP_CMS      0.04f   /* 模式 4 减速步长 */
...
static void OLED_ShowFrozenRuntime(uint32_t frozen_elapsed_tenths)
{
    OLED_ShowNum(2, 7, (frozen_elapsed_tenths / 10U) % 1000U, 3);
    OLED_ShowNum(2, 11, frozen_elapsed_tenths % 10U, 1);
}
...
static void Run_Mode2_IR_Tracking(void)
{
    ...
    while (1)
    {
        ...
        /* 每次循环都读取并清除单次事件标志，防止旧事件在减速后被误判 */
        marker_event = LineTracking_GetMarkerEvent();
        ...
        /* 仅在减速后才识别停止线 */
        if ((speed_reduced != 0U) && (marker_event != 0U))
        {
            LineTracking_Stop();
            OLED_ShowRuntime(start_tick);
            OLED_ShowDistance();
...
static void Run_Mode4_Distance_Test(void)
{
    ...
    uint32_t last_tracking_tick;
    uint32_t last_runtime_refresh_tick;
    uint32_t frozen_elapsed_tenths = 0U;          /* 到达 150cm 时冻结的时间快照 */
    
    uint8_t timer_stopped = 0U;                   /* 150cm 到达标志（同时兼作时间锁存标志） */
    float current_speed = MODE4_START_SPEED_CMS;  /* 模式4专有起步初速度 */
    ...
    OLED_ShowString(3, 1, "Dist: 000.0cm   ");
    OLED_ShowString(4, 1, "Speed: 00cm/s   ");
    OLED_ShowNum(4, 8, (uint32_t)MODE4_START_SPEED_CMS, 2);
    OLED_ShowRuntime(start_tick);
    OLED_ShowDistance();
    ...
    /* 3. 复位编码器并启动底层循迹算法 */
    Odometer_Reset();
    LineTracking_Init(MODE4_START_SPEED_CMS);
    LineTracking_Start();
    ...
    while (1)
    {
        ...
        /* 5. 监测里程计脉冲：行驶达到 150cm 时，立即冻结时间快照 */
        if ((timer_stopped == 0U) && (Odometer_Get_Distance_CM() >= MODE4_TEST_DISTANCE_CM))
        {
            timer_stopped = 1U;
            frozen_elapsed_tenths = (current_tick - start_tick) / 10U;
            OLED_ShowFrozenRuntime(frozen_elapsed_tenths);
        }

        /* 6. 斜坡速度规划控制 */
        if (Odometer_Get_Distance_CM() < MODE4_DECEL_DISTANCE_CM)
        {
            /* 匀速/加速段：斜坡提速至 30cm/s 目标巡航车速 */
            if (current_speed < MODE4_TEST_SPEED_CMS)
            {
                current_speed += MODE4_ACCEL_STEP_CMS;
                if (current_speed > MODE4_TEST_SPEED_CMS)
                {
        ...
        {
            /* 刹车段：超过 200cm 后斜坡减速，降至 10cm/s 后完全停车 */
            current_speed -= MODE4_DECEL_STEP_CMS;
...
static void Run_Mode5_6_IR_Tracking(uint8_t mode)
{
    ...
    uint32_t start_tick;
    uint32_t last_runtime_refresh_tick;
    uint32_t frozen_elapsed_tenths = 0U;  /* 撞线时冻结的时间快照 */
    ...
    while (1)
    {
        ...
        /* 每次循环都读取并清除单次事件标志，防止旧事件在25秒后被误判 */
        marker_event = LineTracking_GetMarkerEvent();

        /* 2. 检测到达停止线（运行25秒后遇到横向标线即认定一圈结束） */
        if (timer_stopped == 0U)
        {
            if (((current_tick - start_tick) >= 2500U) && (marker_event != 0U))
            {
                timer_stopped = 1U;
                frozen_elapsed_tenths = (current_tick - start_tick) / 10U;
                OLED_ShowFrozenRuntime(frozen_elapsed_tenths);
